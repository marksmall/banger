...
class TsCompileTaskLoader extends AbstractTaskLoader {

  constructor (phase) {
    super(phase)
    this.name = 'ts-compile'
    this.phase = phase
  }
  ...
  registerTask (gulp) {
    ...
    gulp.task(this.name, false, () => {
      gutil.log(gutil.colors.green('Transpile TypeScript to ES, include references to library and app .d.ts files and generate sourcemaps'))
...
module.exports = new TsCompileTaskLoader('compile')
